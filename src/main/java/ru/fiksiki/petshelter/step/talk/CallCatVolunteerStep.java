...
import ru.fiksiki.petshelter.keyboard.volunteer.CatVolunteerAcceptKeyBoard;
...
@Service
public class CallCatVolunteerStep extends Step {
    ...
    public CallCatVolunteerStep(StepsContainer container, SendMessageService sendBotMessageService, VolunteerCatService volunteerCatService) {
    ...
    public void startStep(Update update) {
        ...
        CallCatVolunteerStep callCatVolunteerStep = new CallCatVolunteerStep(getContainer(),
                getSendMessageService(),
                volunteerCatService);
        callCatVolunteerStep.setStep(StepName.ZERO);
        getContainer().putStep(id, callCatVolunteerStep);
        SendMessage message = new SendMessage();
        message.setChatId(id);
        message.setText(START_TEXT);
        getSendMessageService().sendMessage(message);

        List<VolunteerCat> volunteers = volunteerCatService.getAll();
        ...
        volunteers.stream().forEach(v -> {
            ...
            alarmMessage.setReplyMarkup(new CatVolunteerAcceptKeyBoard().getKeyBoard(id));
    ...
    public void doStep(Update update) {
        CallCatVolunteerStep currentStep = (CallCatVolunteerStep) getContainer().getStep(update);
